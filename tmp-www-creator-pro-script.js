...
function flatBlocksToEpisodeSource(source){
  ...
  const sceneShotText=(scene)=>{
    const seconds=Number.isFinite(Number(scene?.duracion_segundos))?`${Number(scene.duracion_segundos).toFixed(Number(scene.duracion_segundos)%1?1:0)}s`:"0s";
    const text=String(scene?.toma||scene?.accion||scene?.contenido||scene?.descripcion||scene?.plano||"").trim()||"Toma sin definir";
    return `${seconds} - ${text}`;
  };
  const normalizeText=(value)=>String(value||"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase();
  const sceneSubject=(scene)=>{
    const raw=scene.titulo||scene.nombre||scene.accion||scene.contenido||"";
    return String(raw)
      .replace(/^mostrar\s+/i,"")
      .replace(/\s+con detalle real\.?$/i,"")
      .replace(/\s+de manera concreta\.?$/i,"")
      .trim();
  };
  const storySceneAction=(scene,block,source)=>{
    const original=scene.accion||scene.contenido||"";
    const subject=sceneSubject(scene);
    const sceneText=normalizeText([subject,original,scene.titulo,scene.funcion_narrativa,scene.emocion].filter(Boolean).join(" "));
    const contextText=normalizeText([block?.nombre,block?.objetivo_narrativo,source?.titulo_provisional].filter(Boolean).join(" "));
    const text=[sceneText,contextText].join(" ");
    const generic=/^mostrar\s+.+\s+(con detalle real|de manera concreta)\.?$/i.test(original)||/^mostrar\s+mostrar\s+/i.test(original);
    if(!generic&&original)return original;
    const key=normalizeText(subject);
    if(key.includes("cinco prendas"))return "Mostrar las cinco prendas juntas en mesa, contando visualmente que el lote casi esta listo para entregar.";
    if(key.includes("una separada"))return "Separar la prenda que puede fallar y dejar claro que no esta al mismo nivel que las otras cuatro.";
    if(key.includes("cliente probando"))return "Mostrar al cliente probandose la prenda y buscar una reaccion honesta, no una pose perfecta.";
    if(key.includes("problema talle"))return "Mostrar el problema de talle comparando lo pedido con como queda realmente puesto.";
    if(key.includes("comprador ausente"))return "Mostrar intento de contacto con el comprador y el hueco que deja si no aparece.";
    if(key.includes("pago incompleto"))return "Mostrar el pago incompleto o saldo pendiente y marcar cuanto falta para cerrar esa entrega.";
    if(key.includes("fotos clientes"))return "Mostrar fotos o reacciones de clientes para convertir la entrega en prueba social.";
    if(key.includes("costo real"))return "Mostrar el costo real en numeros y conectarlo con cuanto margen queda despues del error.";
    if(/nombres|lista pedidos|pedidos/.test(sceneText))return "Mostrar la lista real de compradores: nombre, prenda pedida, estado del pago y qué falta entregar.";
    if(/talles|talle/.test(sceneText))return "Comparar el talle pedido con la etiqueta/prenda en mano para que se entienda si puede fallar la entrega.";
    if(/saldos|saldo|pago incompleto|pago/.test(sceneText))return "Mostrar pagos recibidos y saldos pendientes, marcando qué pedido todavía no está cerrado.";
    if(/mapa|trayecto|llegada/.test(sceneText))return "Marcar en mapa o recorrido el camino de entregas, con el orden de paradas y el tiempo que aprieta.";
    if(/packaging|paquete|empaque/.test(sceneText))return "Armar los paquetes uno por uno, con nombre/pedido visible y chequeo rápido antes de salir.";
    if(/numerar|numero/.test(sceneText))return "Numerar cada pedido y cruzarlo con la lista para que la editora vea el sistema de control.";
    if(/mensaje costurero|leer mensaje|costurero/.test(sceneText))return "Abrir el mensaje o audio del costurero y mostrar la parte que cambia el plan de producción.";
    if(/molde/.test(sceneText))return "Poner el molde sobre la prenda y señalar visualmente por qué no sirve para todos los talles.";
    if(/prenda deformada|deformacion|defecto|problema talle|mostrar defecto/.test(sceneText))return "Mostrar el defecto exacto de la prenda con una comparación antes/después o contra el pedido correcto.";
    if(/mensaje compradores|compradores/.test(sceneText))return "Mostrar mensajes de compradores esperando respuesta para que se sienta la presión de cumplir.";
    if(/pedido devolucion|devolucion/.test(sceneText))return "Mostrar el pedido de devolución o reclamo y cortar sobre tu reacción antes de resolverlo.";
    if(/venta extra|transferencia|reserva/.test(sceneText))return "Mostrar la venta o transferencia entrando como posible salvavidas del problema.";
    if(/muestra corregida|corregida|corregir/.test(sceneText))return "Comparar la versión corregida con la fallada y explicar qué cambió para salvar la entrega.";
    if(/foto lote|lote|cinco prendas|prendas mesa/.test(sceneText))return "Mostrar el lote completo ordenado en mesa, contando qué está listo y qué todavía preocupa.";
    if(/una separada|separar problema/.test(sceneText))return "Separar la prenda problemática del resto para que se vea cuál puede arruinar la entrega.";
    if(/cliente probando|probando/.test(sceneText))return "Mostrar al cliente probándose la prenda y buscar una reacción honesta, no una pose perfecta.";
    if(/comprador ausente|ausente/.test(sceneText))return "Mostrar intento de contacto con el comprador y el hueco que deja si no aparece.";
    if(/fotos clientes|fotos/.test(sceneText))return "Mostrar fotos o reacciones de clientes para convertir la entrega en prueba social.";
    if(/costo real|costo/.test(sceneText))return "Mostrar el costo real en números y conectarlo con cuánto margen queda después del error.";
    if(/opcion seguir|seguir/.test(sceneText))return "Plantear visualmente la opción de entregar igual y el riesgo de quedar mal.";
    if(/opcion corregir|elegir corregir|rehacer/.test(sceneText))return "Plantear la opción de corregir aunque cueste tiempo y plata, mostrando la decisión.";
    if(/comparar retraso|retraso/.test(sceneText))return "Mostrar calendario o mensajes para que se entienda cuánto retrasa corregir el problema.";
    if(/talle/.test(contextText)&&/problema talle|defecto|probando/.test(sceneText))return "Mostrar como esta escena afecta la entrega del talle problematico.";
    if(subject)return `Mostrar ${subject} conectado a la consecuencia del bloque, evitando planos de relleno.`;
    return original||"Mostrar la acción clave del bloque y la consecuencia que deja para la siguiente escena.";
  };
  const storyScenePlano=(scene)=>{
    const text=normalizeText(sceneSubject(scene));
    if(scene.plano&&scene.plano!=="plano sugerido")return scene.plano;
    if(/mensaje|pago|saldo|transferencia|mapa|foto/.test(text))return "captura clara / inserto de pantalla";
    if(/talle|molde|defecto|prenda|costura|estampa|etiqueta/.test(text))return "detalle de prenda con mano señalando";
    if(/cliente|comprador|probando|entrega/.test(text))return "plano medio con reaccion visible";
    if(/lote|packaging|paquete|prendas mesa|cinco prendas/.test(text))return "plano cenital de mesa";
    return "plano funcional, cerca de la accion";
  };
  const sceneMain=(scene,block,source)=>{
    const action=storySceneAction(scene,block,source);
    const title=scene.titulo||scene.nombre||sceneSubject(scene);
    const plano=storyScenePlano(scene);
    return [title&&action&&!normalizeText(action).includes(normalizeText(title))?`${title}: ${action}`:action||title, plano?`(${plano})`:""].filter(Boolean).join(" ");
  };
  const sceneDetail=(scene,block,source)=>{
    const parts=[
      scene.titulo?`Titulo: ${scene.titulo}`:"",
      `Plano: ${storyScenePlano(scene)}`,
      `Accion: ${storySceneAction(scene,block,source)}`,
    ...
    return `${scene.numero?`${scene.numero}. `:""}${sceneSeconds(scene)}${parts||sceneMain(scene,block,source)}`;
  };
  const blockFeeling=(block,escenas)=>{
    ...
    if(/gancho|curiosidad|suspenso|corte|ocultar|sin revelar|pendiente|notificaci[oó]n/.test(text))return "Curiosidad con presión: sentir que hay una promesa concreta y necesitar ver qué pasa después.";
    if(/rechazo|no pag|no cierra|consulta|duda|confusi[oó]n|critica|cr[ií]tica/.test(text))return "Incomodidad útil: entender que el avance no está asegurado y preguntarse si la estrategia va a aguantar.";
    if(/problema|mal|deformada|devoluci[oó]n|talle|ausente|incompleto|riesgo/.test(text))return "Tensión real: sentir que una falla chica puede romper todo el progreso conseguido.";
    if(/venta|reserva|transferencia|pago|contador|meta|cinco|5\/5|racha/.test(text))return "Impulso de logro: querer que cierre la venta y sentir que cada número cambia el futuro del reto.";
    if(/decidir|decisi[oó]n|elegir|producto|muestra|validaci[oó]n|opini[oó]n/.test(text))return "Participación mental: sentir ganas de opinar y comparar si esa decisión tiene sentido.";
    if(/entrega|cliente|probando|fotos|comprador/.test(text))return "Expectativa humana: mirar la reacción del cliente como prueba final de si todo valió la pena.";
    if(/costo|dinero|producci[oó]n|costurero|molde|lote/.test(text))return "Presión de negocio: sentir que el reto ya no es una idea, sino una decisión con plata y consecuencias.";
    return "Atención activa: sentir que el bloque deja una pregunta clara y empuja a seguir mirando.";
  };
  ...
  const dias=dayNumbers.map((dayNumber)=>({
    ...
    bloques:source.bloques
      ...
      .map((block,index)=>{
        ...
        const escena=lineText(escenas,(scene)=>sceneDetail(scene,block,source))||block.escena||block.contenido||"";
        ...
        const shotSource=escenas.length
          ? escenas.map(sceneShotText)
          : Array.isArray(block.tomasBloque)&&block.tomasBloque.length
            ? block.tomasBloque
        ...
        return {
          ...
          referenciasTomas:Array.isArray(block.referenciasTomas)?block.referenciasTomas:Array.isArray(block.recursos_usados)?block.recursos_usados:[],
  ...
  return {
    ...
    tomasEpisodio:Array.isArray(source.tomasEpisodio)?source.tomasEpisodio:Array.isArray(source.recursos_generales)?source.recursos_generales.map((resource)=>({id:String(resource.id||slugify(resource.nombre||resource.tipo||"recurso")),nombre:resource.nombre||resource.id||"Recurso",nivel:"episodio",estado:"pendiente",tipo:resource.tipo||""})):[],
    dias
  };
}
...
function hasUndefinedShots(episode){
  return allBlocks(episode).some((block)=>blockSpecificShots(block).some((shot)=>String(shot.nombre||"").includes("Toma sin definir")));
}
async function repairUndefinedShotsFromBundled(episode){
  const number=Number(episode?.episodio||0);
  if(!number||!hasUndefinedShots(episode))return false;
  let official;
  try{
    official=normalizeEpisode(await fetchBundledEpisodeNumber(number));
  }catch(_error){
    return false;
  }
  const byId=new Map(allBlocks(official).map((block)=>[block.id,block]));
  const byPosition=new Map(allBlocks(official).map((block)=>[`${block.dia}-${block.orden}`,block]));
  let repaired=false;
  allBlocks(episode).forEach((block)=>{
    if(!blockSpecificShots(block).some((shot)=>String(shot.nombre||"").includes("Toma sin definir")))return;
    const source=byId.get(block.id)||byPosition.get(`${block.dia}-${block.orden}`);
    if(!source||!blockSpecificShots(source).length)return;
    block.tomasBloque=blockSpecificShots(source).map((shot)=>normalizeShot({...shot,id:`${block.id}-${shot.id}`},"bloque"));
    repaired=true;
  });
  if(repaired)episode.updated_at=new Date().toISOString();
  return repaired;
}
async function repairAllUndefinedShots(){
  let count=0;
  for(const episode of episodes){
    if(await repairUndefinedShotsFromBundled(episode))count+=1;
  }
  if(count){
    persistLocal();
    setStatus(`Repare tomas sin definir en ${count} episodio${count===1?"":"s"}. Toca Guardar para subirlo a Supabase.`);
  }
}
...
async function saveActive(options={}){
  ...
  await repairUndefinedShotsFromBundled(episode);
